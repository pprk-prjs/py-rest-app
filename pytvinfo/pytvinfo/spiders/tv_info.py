...
from pytvinfo.items import PyTvIdItem
import re
...
class WebTvInfo:
    ...
    def __init__(self):
        ...
        _tomorrow = pu.time_str(self.tomorrow, '%Y-%m-%d')        
        #today link
        ...
        #tomorrow link
        self.start_urls.append('http://www.tvprogram.cz/index.php?P_id_kategorie=56456&P_soubor=televize%2Findex.php%3Fdatum%3D{0}%26zobrazeni%3D%26typprg_pouze%3D%26casod%3D-1%26typzanr%3D'.format(_tomorrow))
...
#         if self._tv_list == None:
        self._tv_list = tab_tv_id
...
#             print(">>>>>>>>>>>>>>>>", name)
        ...
        for indx, _tv_id in enumerate(tab_tv_id):
            if _tv_id in self._tv_list:
                tv_info_data = PyTvIdItem()
                ...
                for part, tds in tabs_tds:
                    ...
                    for tab in tabs_program:
                        item = self.__parse_table(tab)
                        item['tv_id'] = tv_id
                        info_list.append(item)
                tv_info_data[tv_id] = info_list
                yield tv_info_data  
    ...
    def __get_start_date(self, cas, end):
        cas = cas.xpath('text()').extract()[0]
        cas = cas.split('.')
        h = int(cas[0])
        m = int(cas[1])
        cas = end.replace(hour=h, minute=m)
        if cas > end: 
...
#         print("TIME::::", _time)
        _endtime = table.xpath('@data-k')[0]
        _endtime = _endtime.extract()
#         print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<", _endtime)
        end = pu.str_time(_endtime,"%Y%m%d %H%M")
        cas = self.__get_start_date(_time, end)
#         print(cas)
...
class TvInfoTomorrowScrapy(TvInfoScrapy):
    name = 'epg_tm'

    def start_requests(self):
        yield scrapy.Request(self.urls.start_urls[1], cookies=self.urls.cookies, encoding='cp1250')

        
class TvInfoTodayScrapy(TvInfoScrapy):
    name = 'epg_td'

    def start_requests(self):
        yield scrapy.Request(self.urls.start_urls[0], cookies=self.urls.cookies, encoding='cp1250')
